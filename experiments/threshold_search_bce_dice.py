...
CSV_PATH = RESULT_DIR / "threshold_search_bce_dice.csv"
JSON_PATH = RESULT_DIR / "threshold_search_bce_dice.json"
...
def save_results(
    ...
    output = {
        ...
        "bce_weight": checkpoint.get("bce_weight"),
        "dice_weight": checkpoint.get("dice_weight"),
        "checkpoint_val_dice_at_0_5": checkpoint.get(
            "val_dice"
        ),
...
def main() -> None:
    ...
    print(
        f"Loss: "
        f"{checkpoint.get('loss_name', 'unknown')}"
    )
    print(
        f"Validation images: "
        f"{len(val_dataset)}"
    ...
    print("=== Threshold Search: BCE + Dice ===")
    print(
        f"{'Threshold':>10} | "
        f"{'Dice':>8} | "
        f"{'IoU':>8}"
